/**
 * Landing page HTML template for the facilitator API.
 * Extracted from index.ts to keep route handlers focused on logic.
 */
export function landingPageHtml(env: { NETWORK: string; OUR_ADDRESS: string }): string {
  return `<!DOCTYPE html>
<html lang="en"><head>
...
<div class="info">
<div class="info-item"><div class="info-label">Network</div><div class="info-value">${env.NETWORK}</div></div>
<div class="info-item"><div class="info-label">Address</div><div class="info-value"><code>${env.OUR_ADDRESS.slice(0, 6)}...${env.OUR_ADDRESS.slice(-4)}</code></div></div>
