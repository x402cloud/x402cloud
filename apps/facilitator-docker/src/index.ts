...
// Allow auth-off only when explicitly opted in (e.g. local dev behind
// an authenticating gateway). Default is fail-closed so a missing token
// never accidentally exposes /verify or /settle to the public network.
const ALLOW_NO_AUTH = process.env.FACILITATOR_ALLOW_NO_AUTH === "true";

if (!PRIVATE_KEY) throw new Error("FACILITATOR_PRIVATE_KEY is required");
if (!RPC_URL) throw new Error("RPC_URL is required");
if (!API_TOKEN && !ALLOW_NO_AUTH) {
  throw new Error(
    "FACILITATOR_API_TOKEN is required. Set FACILITATOR_ALLOW_NO_AUTH=true only " +
      "for local dev or when running behind an authenticating gateway.",
  );
}
...
// ── Auth middleware (fail-closed unless FACILITATOR_ALLOW_NO_AUTH=true) ─
...
// Bind auth directly to the payment routes so it cannot be omitted at the
// mount site (see security review).
app.route(
  "/",
  createFacilitatorRoutes(() => facilitator, { auth: authMiddleware }),
);
