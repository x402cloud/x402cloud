...
// ../../packages/probes/dist/targets.js
var TARGETS = {
  local: {
    name: "local",
...
// ../../packages/probes/dist/run.js
async function runProbes(probes, target) {
...
__name(formatEth, "formatEth");

// ../../packages/probes/dist/probes/index.js
var allProbes = [
  rpcAlive,
  usdcContract,
  permit2Contract,
...
__name(truncateValue, "truncateValue");
...
function renderProbeRow(result) {
  const color = STATUS_COLORS[result.status];
  const errorHtml = result.error ? `<div class="error">${escapeHtml(result.error)}</div>` : "";
...
__name(renderProbeRow, "renderProbeRow");
function renderTargetSelector(activeTarget, availableTargets) {
  return availableTargets.map((name) => {
...
__name(renderTargetSelector, "renderTargetSelector");
...
function renderDashboard(report, availableTargets) {
...
// src/index.ts
function resolveTarget(name, env) {
...
app.get("/", async (c) => {
  ...
  const host = c.req.header("host") ?? "";
  const isLocal = host.startsWith("localhost") || host.startsWith("127.0.0.1");
  const visibleTargets = isLocal ? Object.keys(TARGETS) : Object.keys(TARGETS).filter((t) => t !== "local");
  const report = await runProbes(allProbes, target);
...
// ../../node_modules/.pnpm/wrangler@4.68.0_@cloudflare+workers-types@4.20260302.0/node_modules/wrangler/templates/middleware/middleware-ensure-req-body-drained.ts
...
// ../../node_modules/.pnpm/wrangler@4.68.0_@cloudflare+workers-types@4.20260302.0/node_modules/wrangler/templates/middleware/middleware-miniflare3-json-error.ts
...
// .wrangler/tmp/bundle-j85ekZ/middleware-insertion-facade.js
...
// ../../node_modules/.pnpm/wrangler@4.68.0_@cloudflare+workers-types@4.20260302.0/node_modules/wrangler/templates/middleware/common.ts
...
// .wrangler/tmp/bundle-j85ekZ/middleware-loader.entry.ts
