...
import { scrapeManifest, scrapeEntries } from "../src/index.js";
...
const params: ManifestParams = {
  ...
  baseUrl: "https://scrape.x402cloud.ai",
};

describe("scrapeManifest", () => {
  it("returns at least one service", () => {
    expect(scrapeManifest(params).length).toBeGreaterThan(0);
  });

  it("every entry uses the operator wallet as payTo", () => {
    for (const s of scrapeManifest(params)) {
      expect(s.payment.payTo).toBe(params.payTo);
    }
  });

  it("entries(p) and manifest(p) agree on id+maxPrice pairs", () => {
    const cat = scrapeManifest(params);
    const ent = scrapeEntries(params);
